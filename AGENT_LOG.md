...
- **Verification / review:** Final controller evidence on the committed tree: `npm.cmd run test -- --run` reported `3 passed` files and `10 passed` tests; `npm.cmd run typecheck` exited 0; `git diff --check` reported no whitespace errors. The client posts UTF-8 bytes only to `/api/v1/reviews`, exposes only stable public error codes, preserves native keyboard form controls, disables duplicate submits while pending, and never surfaces raw error bodies. A fresh spec and quality review dispatch was attempted once but the platform rejected it with `agent thread limit reached`; it was not retried. The controller instead performed a documented local review against `SPEC.md`, `PLAN.md`, both T13 design contracts, the actual FastAPI endpoint, and the pinned UI guidelines. This is a task-local review-process deviation and must not be represented as a fresh-agent approval.
- **Commit / boundary:** `27f1a61` (`feat(web): add diff input workflow`) is the real local code commit. No remote repository was pushed and no deployment, registry publication, or real OpenAI request occurred.
