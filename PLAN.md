...
| T05.7 | **CANCELLED / REMOVED BY APPROVED SCOPE REVISION（2026-08-12）。** JS-007 不属于首期正式规则集，不得作为完成任务或正式能力交付。真实证据 `3852a88` 记录：在禁止 tokenizer、AST、TypeScript Compiler 和外部解析依赖的边界内，独立审查复现了 TSX 嵌套 JSX、泛型箭头函数及嵌套对象默认值的可靠区分冲突。用户明确拒绝引入解析子系统并选择移除这一 Low 规则。先前 `codex/js-rules-risk` 和 `codex/js-any-narrow` 的 JS-007 尝试仅保留为过程证据，禁止复用为正式实现。 | 不适用；不得伪造 RED/GREEN 或完成状态。范围修订后，正式 v1 必须没有 JS-007 metadata、scanner、测试或文档宣称。 | 不适用；执行范围为撤除未批准 JS-007 资产并通过全量一致性检查，随后进入 T05.8。 |
