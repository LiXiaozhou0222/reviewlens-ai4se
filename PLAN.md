...
| T05.3 | 同上：仅 JS-003 `eval()`。<br>**真实完成提交：** `99657fd` (`feat(api): detect direct JS eval calls`)；2026-08-11 经规约复审、代码质量修复与 scoped 复审批准。 | 计划测试名与实际 `test_js_003_finds_added_eval` 不同；初始 Python 3.12 RED 为缺少 `scan_js_003`。后续回归覆盖方法声明、JSX 文本与控制流调用边界。 | 实际节点：`1 passed`；JS 测试：`33 passed`；后端：`102 passed`，均为 Python 3.12。 |
| T05.4 | 同上：仅 JS-004 `innerHTML`/`dangerouslySetInnerHTML`。<br>**真实完成提交：** `0d8adb7` (`feat(api): detect JS HTML injection sinks`)；2026-08-11 经规约审查、四轮质量修复与 scoped 复审批准。 | 初始 Python 3.12 RED：缺少 `scan_js_004`；后续覆盖 bare binding、multi-line JSX、JSX expression comparison 与 expression-depth 边界。 | 最终 Python 3.12：聚焦 `2 passed`；JS 测试 `49 passed`；后端 `118 passed`。 |
| T05.5 | 同上：仅 JS-005 空/吞异常 `catch`；允许上下文但只能锚定新增行。<br>**真实完成提交：** `f9cc688` (`feat(api): detect swallowed JS catches`)；2026-08-11 经规约复审与代码质量审查批准。 | 实际 Python 3.12 RED：缺少 `scan_js_005`；修复 RED：一行 fully-added `catch {}` 未命中。 | 实际节点 `test_js_005_anchors_empty_catch_to_added_line`：`1 passed`；修复节点：`1 passed`；JS `56 passed`；后端 `125 passed`。 |
| T05.6 | 同上：仅 JS-006 可明确识别的未 `await`/未 `return`/未处理 `fetch()`。<br>**真实完成提交：** `9cdd99c` (`feat(api): detect unhandled JS fetch calls`)；2026-08-11 经规约符合性与代码质量审查批准。 | 计划预填测试名与实际 `test_js_006_finds_unhandled_added_fetch` 不同；初始 Python 3.12 RED：缺少 `scan_js_006`。 | 实际节点：`1 passed`；JS 规则：`64 passed`；完整后端：`133 passed`，均为 Python 3.12。 |
| T05.7 | **2026-08-12 人工收窄后从批准基线重做。** 新 worktree/branch：`codex/js-any-narrow`，基线 `98916c8`；旧 `codex/js-rules-risk` 的未批准 JS-007 尝试只保留为过程证据，禁止复用其实现 session 或代码。仅实现 JS-007 Low/language_rule/v1.0.0：新增非二进制 `.ts`/`.tsx` 行中的高置信变量 `const`/`let`/`var <identifier>: any`、具名函数/箭头函数的参数或返回 `: any`、以及完整调用/索引表达式后的 `) as any` / `] as any`。不得实现 tokenizer、AST、TypeScript Compiler、外部解析依赖、interface/type object member 或跨行 assertion 支持。 | fresh implementation subagent 先只新增 `test_js_007_finds_narrowed_explicit_any` 和负例测试；控制器从 `apps/api` 用 `py -3.12 -m pytest tests/rules/test_javascript_rules.py::test_js_007_finds_narrowed_explicit_any -q` 记录 RED（`scan_js_007` 缺失）。负例至少覆盖 JSX 文本、字符串、模板文本、正则字面量、`import`/`export` alias、语句标签、对象字面量、`.js`/`.jsx`、删除/上下文行、binary 与不支持路径。 | 同一 Python 3.12 节点 `1 passed`；随后 JS 规则文件与完整后端均通过。每个 Finding 必须是 JS-007/Low/language_rule/v1.0.0，锚定新增 `.ts`/`.tsx` 行。完成后依次进行规约符合性审查和代码质量审查；任何 Critical/Important 必须在本新任务循环内关闭。 |
