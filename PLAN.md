...
| --- | --- | --- | --- |
| T02.1 | `app/models/domain.py`、`tests/models/test_domain_contracts.py`：只定义 `ReviewMode`、Severity、Source、AI 状态。 | `cd apps/api; pytest tests/models/test_domain_contracts.py::test_review_mode_and_severity_values_are_fixed -q`；枚举或值缺失。 | 同一命令；`1 passed`。 |
| T02.2 | `app/models/errors.py`、`tests/models/test_error_contracts.py`：只定义稳定公开错误代码。 | `cd apps/api; pytest tests/models/test_error_contracts.py::test_input_error_codes_are_stable -q`；代码缺失或变化。 | 同一命令；`1 passed`。 |
...
| Task | 文件与最小目标 | RED / 预期 | GREEN / 预期 |
| --- | --- | --- | --- |
| T03.1 | `diff_parser/normalizer.py`、`tests/diff_parser/test_normalizer.py`：空输入与 UTF-8 拒绝。 | `cd apps/api; pytest tests/diff_parser/test_normalizer.py::test_rejects_empty_input -q`；normalizer 缺失或未拒绝。 | 同一命令；`1 passed`。 |
| T03.2 | 同上：BOM 去除、CRLF/CR→LF 与 SHA-256 等价。 | `cd apps/api; pytest tests/diff_parser/test_normalizer.py::test_crlf_and_lf_have_the_same_digest -q`；摘要不同。 | 同一命令；`1 passed`。 |
| T03.3 | 同上：500 KB、5,000 行硬拒绝，无静默截断。 | `cd apps/api; pytest tests/diff_parser/test_normalizer.py::test_rejects_over_line_limit -q`；未返回限制错误。 | 同一命令；`1 passed`。 |
| T03.4 | `diff_parser/parser.py`、`tests/diff_parser/test_parser.py`：合法 unified Diff 与新增行的新文件行号。 | `cd apps/api; pytest tests/diff_parser/test_parser.py::test_maps_added_line_to_new_file_line_number -q`；parser/行号映射缺失。 | 同一命令；`1 passed`。 |
| T03.5 | 同上：重命名、删除、`+++` 头与 binary 元数据。 | `cd apps/api; pytest tests/diff_parser/test_parser.py::test_file_header_is_not_an_added_code_line -q`；头被当新增代码。 | 同一命令；`1 passed`。 |
| T03.6 | 同上：非 unified Diff 的明确拒绝且不创建报告。 | `cd apps/api; pytest tests/diff_parser/test_parser.py::test_rejects_invalid_unified_diff -q`；格式无效内容被接纳。 | 同一命令；`1 passed`。 |
...
| T04.5 | 同上：仅 GEN-004 非 loopback `http://` 硬编码地址。 | `cd apps/api; pytest tests/rules/test_general_rules.py::test_gen_004_ignores_loopback_http -q`；loopback 误报或非 loopback 漏报。 | 同一命令；`1 passed`。 |
| T04.6 | `rules/engine.py`：仅 GEN-005 文件级变更规模，行号为 `null`。 | `cd apps/api; pytest tests/rules/test_general_rules.py::test_gen_005_is_file_level -q`；错误伪造代码行号。 | 同一命令；`1 passed`。 |
